...
```{r: table percentiles std normal dist, cache=TRUE, echo = FALSE, results='asis'}
require(xtable) # for a pretty table in the HTML output
m <- matrix(data=c(-2.33,-1.96,-1.645,-1.28,1.28,1.645,1.96,2.33,1.0, 2.5, 5.0, 10.0, 90.0, 95.0, 97.5, 99.0),nrow=8,dimnames=list(c("","","","","","","",""),c("Z-values","Percentiles")))
xt <- xtable(m,caption="Z-values and their respective percentiles")
digits(xt)[c(1,2,3)]<-c(0,3,1)
display(xt)[c(1,2,3)]<-c("s","f","f")
align(xt)[c(1,2,3)]<-c("l","r","r")
print( xt ,type="html")
```
